...
def evaluate(path_exp_traj, path_lrn_traj, len_lane, max_episode_len, sparse=False):
    ...
    if 'reward' in list(traj_lrn_df):
        ...
        rewardVecExp, rewardTsExp, rewardVecLrn, rewardTsLrn = np.array(rewardVecExp).mean(), \
                                                               np.array(rewardTsExp).mean(), \
                                                               np.array(rewardVecLrn).mean(), \
                                                               np.array(rewardTsLrn).mean()
        print('Reward: expert - [%0.4f, %0.4f], learned - [%0.4f, %0.4f]'
              % (rewardVecExp, rewardTsExp, rewardVecLrn, rewardTsLrn))
    else:
        rewardVecExp, rewardTsExp, rewardVecLrn, rewardTsLrn = 0, 0, 0, 0
    ...
    if sparse:
        ...
        print('Overall RMSE of time =', rmse_time)
    else:
        rmse_time = rmse_eval_time_dense(traj_exp_origin, traj_lrn_origin)
        print('Overall RMSE of time =', rmse_time)
